...
{
    WorldPacket* const rawPacket = new WorldPacket(CMSG_SOCKET_GEMS);
    *rawPacket << item->GetGUID();
    ...
    if (fits)
    {
        ...
        WorldPackets::Item::SocketGems packet(std::move(*rawPacket));
        delete rawPacket;
        packet.Read();
